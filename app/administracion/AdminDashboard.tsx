...
type User = { id: string; email: string; name: string; role: string; professional: string | null; active: boolean };
...
function UsersView({ users, onAction }: { users: User[]; onAction: (payload: Record<string, unknown>) => Promise<boolean> }) {
  ...
  const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [role, setRole] = useState("receptionist"); const [professional, setProfessional] = useState("Kiara Moscoso");
  return <div className="admin-content"><div className="admin-section-head"><div><p>CONTROL DE ACCESO</p><h2>Usuarios y permisos.</h2></div><button className="admin-primary" onClick={() => setFormOpen(!formOpen)}><FiPlus /> Nuevo usuario</button></div>{formOpen && <form className="admin-inline-form" onSubmit={async (event) => { event.preventDefault(); if (await onAction({ action: "add_user", name, email, role, professional })) { setFormOpen(false); setName(""); setEmail(""); } }}><label>Nombre<input value={name} onChange={(event) => setName(event.target.value)} required /></label><label>Correo de acceso<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Rol<select value={role} onChange={(event) => setRole(event.target.value)}><option value="location_admin">Administradora del centro</option><option value="receptionist">Recepción</option><option value="professional">Profesional</option><option value="readonly">Solo lectura</option></select></label>{role === "professional" && <label>Agenda asignada<select value={professional} onChange={(event) => setProfessional(event.target.value)}>{professionals.map((item) => <option key={item}>{item}</option>)}</select></label>}<button>Crear acceso</button></form>}<div className="admin-table users-table"><div className="table-head"><span>Usuario</span><span>Rol</span><span>Agenda</span><span>Estado</span><span></span></div>{users.map((user) => <div className="table-row" key={user.id}><span><b>{user.name}</b><small>{user.email}</small></span><span>{roleLabel[user.role] ?? user.role}</span><span>{user.professional ?? "Todas"}</span><span>{user.active ? "Activo" : "Suspendido"}</span><button onClick={() => void onAction({ action: "toggle_user", id: user.id, active: !user.active })}>{user.active ? "Suspender" : "Activar"}</button></div>)}</div></div>;
}
